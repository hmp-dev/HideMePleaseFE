...
import 'package:mobile/features/wepin/widgets/transaction_dialog.dart';
import 'package:mobile/features/wepin/widgets/user_drawer.dart';
...
class WepinSDKDemoScreen extends StatefulWidget {
  const WepinSDKDemoScreen({super.key, required this.googleAuthAccessToken});

  final String googleAuthAccessToken;
  @override
  WepinSDKDemoScreenState createState() => WepinSDKDemoScreenState();
}

class WepinSDKDemoScreenState extends State<WepinSDKDemoScreen> {
  ...
  Future<void> loginWithProvider(String provider, String? clientId) async {
    await performActionWithLoading(() async {
      try {
        final oauthToken = await wepinSDK!.login
            .loginWithOauthProvider(provider: provider, clientId: clientId!);

        "the token type is ${oauthToken?.type}".log();
        "the token type is ${oauthToken?.token}".log();

        final idToken = oauthToken?.token;
        // final sign = wepinSDK?.login.getSignForLogin(
        ...
        if (type == WepinOauthTokenType.idToken) {
          fbToken = await wepinSDK!.login.loginWithIdToken(idToken: idToken!);
        } else {
          fbToken = await wepinSDK!.login
              .loginWithAccessToken(provider: provider, accessToken: idToken!);
        }
  ...
  Future<void> loginWithGoogleProvider(
      String provider, String? clientId) async {
    await performActionWithLoading(() async {
      try {
        LoginResult? fbToken;
        ...
        if (Platform.isAndroid) {
          fbToken = await wepinSDK!.login.loginWithAccessToken(
              provider: provider, accessToken: widget.googleAuthAccessToken);
        }
        ...
        if (fbToken != null) {
          ...
          if (wepinUser != null && wepinUser.userInfo != null) {
            ...
            showError('Login Failed. No user info found.');
          }
        } else {
  ...
  Future<void> loginWithEmail(String email, String password) async {
    await performActionWithLoading(() async {
      try {
        final fbToken = await wepinSDK!.login
  ...
  Future<void> signUpWithEmail(String email, String password) async {
    await performActionWithLoading(() async {
      try {
        final fbToken = await wepinSDK!.login
            .singUpWithEmailAndPassword(email: email, password: password);
        final wepinUser = await wepinSDK?.login.loginWepin(fbToken);
        userEmail = wepinUser!.userInfo!.email;
        wepinStatus = await wepinSDK!.getStatus();
      } catch (e) {
    ...
    return Scaffold(
              ...
              children: [
                if (wepinSDK != null) ...[
                  ...
                  Card(
                    ...
                    child: ListTile(
                      title: Text('Wepin Status: $wepinStatus',
                    ...
                    children: [
                      if (wepinStatus == WepinLifeCycle.initialized) ...[
                        _buildActionButton(
                          'Login',
                          () async {
                            "the idToken is ${widget.googleAuthAccessToken}"
                                .log();
                            var result = await wepinSDK?.login.loginWithIdToken(
                                idToken: widget.googleAuthAccessToken);
                            wepinStatus = await wepinSDK!.getStatus();

                            "the login wepin result is $result".log();
                          },
                        ),
                        ...
                        _buildActionButton('Login With UI(specified Email)',
                          ...
                          showDialog(
                            context: context,
                            builder: (ctx) => EmailLoginDialog(
                                requirePassword: false, onLogin: _loginWithUI),
                          );
                        }),
                        _buildActionButton(
                            'Login with Google',
                            () => loginWithProvider(
                                'google', findClientIdByProvider('google'))),
                        _buildActionButton(
                            'Login with Google Private',
                            () => loginWithGoogleProvider(
                                'google', findClientIdByProvider('google'))),
                        _buildActionButton(
                            'Login with Apple',
                            () => loginWithProvider(
                                'apple', findClientIdByProvider('apple'))),
                      ],
                      if (wepinStatus == WepinLifeCycle.login) ...[
                        _buildActionButton(
                            'Open Widget', () => wepinSDK!.openWidget(context)),
                        _buildActionButton('Get Accounts', () async {
                          await performActionWithLoading(() async {
                            ...
                            for (var account in accountsList) {
                              "the Wallet account is $account".log();
                            }
                        ...
                        if (accountsList.isNotEmpty) ...[
                          ...
                          _buildActionButton('Receive', () async {
                            await navigateToAccountSelection(
                                selection: true,
                          ...
                          _buildActionButton('Get Balance', () async {
                            ...
                            if (selectedAccounts.isNotEmpty) {
                              await performActionWithLoading(() async {
                                balanceList = await wepinSDK!
                                    .getBalance(accounts: selectedAccounts);
                                wepinStatus = await wepinSDK!.getStatus();
                          ...
                          if (balanceList.isNotEmpty)
                            _buildActionButton('Account Balance List View',
                                navigateToBalanceList),
                        ],
      ...
      endDrawer: UserDrawer(
        ...
        onModeChanged: (value) {
          selectedValue = value;
          final selectedConfig =
              sdkConfigs.firstWhere((config) => config['name'] == value);
          _updateConfig(selectedConfig);
          initWepinSDK(selectedConfig['appId']!, selectedConfig['appKey']!,
              selectedConfig['privateKey']!);
        },
        ...
        selectedSocialLogins:
            selectedSocialLogins, // Initially selected social logins
