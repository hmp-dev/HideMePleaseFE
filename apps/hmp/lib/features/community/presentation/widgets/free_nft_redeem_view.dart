...
class FreeNftRedeemView extends StatelessWidget {
                      ...
                      child: Column(
                        ...
                        children: [
                          ...
                          HMPCustomButton(
                            ...
                            text: getButtonTitle(
                                isFreeWelcomeNFTAvailable,
                                walletsState.isWepinWalletConnected ||
                                    walletsState
                                        .isKlipWalletConnected), //'지갑 연결하기',
                            onPressed: () {
                              //
                              final isWelcomeNftRedeemerWalletConnected =
                                  walletsState.isWepinWalletConnected ||
                                      walletsState.isKlipWalletConnected;

                              //
                              if (isFreeWelcomeNFTAvailable &&
                                  isWelcomeNftRedeemerWalletConnected) {
                                // redeem free Welcome NFT
                                getIt<NftCubit>().onGetConsumeWelcomeNft();
                              } else if (isFreeWelcomeNFTAvailable &&
                                  !isWelcomeNftRedeemerWalletConnected) {
                                // open wallet Connect bottom Model
                                getIt<WalletsCubit>()
  ...
  getButtonTitle(
    bool isFreeWelcomeNFTAvailable,
    bool isWelcomeNftRedeemerWalletConnected,
  ) {
    if (isFreeWelcomeNFTAvailable && isWelcomeNftRedeemerWalletConnected) {
      return LocaleKeys.getAFreeNft.tr();
    } else if (isFreeWelcomeNFTAvailable &&
        !isWelcomeNftRedeemerWalletConnected) {
      return LocaleKeys.connectWallet.tr();
    } else if (!isFreeWelcomeNFTAvailable) {
      return LocaleKeys.myMembershipSettings.tr();
    } else {
      return LocaleKeys.myMembershipSettings.tr();
    }
  }
